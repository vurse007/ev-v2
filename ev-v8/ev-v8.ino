...
double metersToTicks(double meters)
{
    ...
    const double ENCODER_CPR = 13;           // counts per revolution // 14
...
class PID {
  public:
    ...
    double calculate(double target, double current, double scale=1.0, bool H=false){
      ...
      if ((prev_error * error) == -1){
        //if there was a sign flip in error reset the integral
        total_error = 0;
      }
...
PID heading_corrector(1.7, 0, 900);
PID distance_corrector(0.0045, 0,0.567);  // Tune these gains for your robot
...
const double TARGET_DISTANCE = metersToTicks(8.25-0.055);  // Adjust this value for desired distance
...
const int SETTLED_COUNT_THRESHOLD = 12000;        // How many consecutive loops within threshold
