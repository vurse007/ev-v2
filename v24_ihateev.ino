...
void driveDistance(double meters, unsigned long timeoutMs, double speedScale) {
  ...
  headingPID.kP = 9.75; // 6
  headingPID.kD = 55; // 467
...
// // minimum drive power MUST happen before mixing
...
// optional tiny kick if real error exists but PID output is too weak
if (fabs(hErr) > 3.0 && fabs(corr) < 0.08) {
  corr = 0.08 * (hErr > 0 ? 1 : -1);
}

// ===================== CLEAN CORRECTION LIMIT =====================
// tie correction to forward drive, but preserve some minimum authority
double maxCorr = fabs(base);
if (maxCorr < 0.18) maxCorr = 0.18;
...
// prevent unintended reversal during a forward/backward drive
if (base > 0) {
  if (left < 0) left = 0;
  if (right < 0) right = 0;
}
...
void setup() {
  ...
  Serial.println("Press button to start (keep robot still)");

  while (digitalRead(START_PIN) == HIGH) {
    // waiting for press
  }

  delay(200); // debounce + let vibrations settle
...
void loop() {
  ...
  driveDistance(0.825, 3000, 1);
  unsigned long start = millis();
  while (millis() - start < 200) {
    updateIMU();
    updateEncoders();
    delay(2);  // keeps ~500 Hz-ish updates, stable
  }
