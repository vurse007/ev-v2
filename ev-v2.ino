...
#define ENC_L 16  // Left encoder pin 
...
PID heading_corrector(0.5, 0, 3);
PID distance_corrector(1, 0.0, 0);  // Tune these gains for your robot
...
const double TARGET_DISTANCE = 30000.0;  // Adjust this value for desired distance
const double MIN_BASE_SPEED = -0.2;      // Minimum speed to keep robot moving
const double MAX_BASE_SPEED = 0.7;      // Maximum speed

// === BREAKOUT CONDITION VARIABLES ===
const double DISTANCE_ERROR_THRESHOLD = 10.0;  // How close to target counts as "arrived"
const int SETTLED_COUNT_THRESHOLD = 50;        // How many consecutive loops within threshold
...
void loop() {
  ...
  if (motorRunning) {
    ...
    double avgDistance = (encoderCountL + encoderCountR * 20.633) / 2.0;
    ...
    double headingCorrection = heading_corrector.calculate(0.0, heading);

    // === USE DISTANCE PID FOR SPEED CONTROL ===
    // Target = TARGET_DISTANCE, Current = avgDistance
    double baseSpeed = distance_corrector.calculate(TARGET_DISTANCE, avgDistance);
    ...
    Serial.print(", EncR: ");
    Serial.print(encoderCountR * 20.633);
    ...
    if (settledCount >= SETTLED_COUNT_THRESHOLD) {
      ...
      Serial.print(encoderCountL);
      Serial.print(", Right: ");
      Serial.println(encoderCountR * 20.633);
